...
export const downloadUrlToPath = async (url: string, destPath: string, electronEvent: IpcMainEvent): Promise<void> => new Promise((resolve, reject) => {
    ...
    window.webContents.session.on('will-download', (_, item) => {
        ...
        item.on('updated', () => {
            if (currentDownloadItem) {
                electronEvent.reply('videos_progress', computePercentageAndRemainingTime());
            }
        });
...
const computePercentageAndRemainingTime = () => {
    if (!currentDownloadItem) {
        return null;
    }
